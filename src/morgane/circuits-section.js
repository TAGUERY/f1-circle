...
function createCircuitsCircle() {
  ...
  circuits.circuits.forEach((url, i) => {
    ...
    image.on("mouseover", function () {
      ...
      d3.selectAll("image:not(:hover)")
        .style("opacity", 0.3);

      circuitLeftImage.attr(
        "xlink:href", url.img_circuit
      );
    ...
    image.on("mouseout", function () {
      d3.select(this)
        ...
        .attr("width", radius / 4) // Réinitialiser la largeur de l'image
        .attr("height", radius / 4);
      d3.selectAll("image")
        .style("opacity", 1); // Réinitialiser l'opacité de toutes les images à 100%
      middleZone.selectAll("*").remove();
      middleZone.append(() => baseImageAboveText.node());
      middleZone.append(() => baseTexteZone.node());
